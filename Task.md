...
> 현재 진척: **Phase 1~3 완료**, Phase 3.5~ 진행 예정.
...
- [x] **Goal 로직**: 공이 닿았을 때 클리어(성공) 판정을 내리는 트리거 구현
- [x] **장애물(Hole/Spike/DeadZone)**: Hole/Spike 및 **장외 이탈(OOB) 감지용 데드존 트리거**와 실패 판정 연동 구현
- [x] **상태 제어 (GameManager)**: 씬 재로드(LoadScene) 없이 상태계(Transform 및 Velocity)만 초기화하는 빠른 재시작(Fast Retry) 및 **물리 가속도(`zero`) 초기화** 로직 구현
- [x] **피드백 적용**: 쿨타임(0.1초) 제한이 포함된 햅틱 진동 및 충돌/성공 효과음(SFX/Particle) 로직 추가

## Phase 3: 색깔 공 기믹 (핵심 퍼즐 요소)
- [x] **다중 공 레이어 설정**: 서로 다른 두 색상의 공을 생성하고, 선택적 통과를 위한 Layer Collision Matrix 분리 구성
- [x] **단일/다중 복합 승리 조건**: 모든 공이 각자의 색상에 맞는 Goal에 들어갔을 때만 스테이지 클리어로 판정하는 알고리즘 작성
...
- [ ] **경제 — 별/화폐 분리**: 성취 기록용 **별(불변)** 과 소비용 **소프트 화폐(Coin/Shard)** 를 별도 필드로 설계·구현 (별은 소모하지 않음)
- [ ] **상점 & 스킨**: 소프트 화폐로 공 Trail·골인 파티클·미로 테마(코스메틱)를 해금하는 상점 구현
- [ ] **리워드 광고(BM)**: 강제 전면광고 지양, "화폐 부족 시 동영상 시청 → 화폐 획득" 형태의 자발적 보상형 광고 연동
...
## 추후 결정 (Deferred — 개발하며 확정)
- 콘텐츠 총량(월드/레벨 수): 데이터 주도(`StageData`)로 점진 확장 (GDD §6.2)
- 최소 지원 기기 사양 / 메모리 예산 (TDD §1)
- 지원 언어 & 현지화(텍스트 외부 테이블화)
- BGM 톤·레퍼런스 (Phase 6에서 구체화)
- 레퍼런스 경쟁작 벤치마크 (GDD §1)
