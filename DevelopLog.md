...
### 📅 [2026-08-15] Phase 3·4 종료 — 햅틱 피드백 배선 · 승/패 우선순위 결정 · 좁은 통로 교착 검증

#### 1. 배경

Phase 3의 완료 조건("골인/실패가 판정되고, 실패 시 씬 재로드 없이 즉시 복원된다")은 2026-08-08 실플레이로 이미 관측됐으나, **완료 조건 밖의 항목 2건**이 남아 Phase가 닫히지 않고 있었다 — 피드백 배선(`[Code]`)과 승/패 동시 발생 우선순위 결정(`[Doc]`).

#### 2. 피드백 배선 — 데드코드 해소

`FeedbackManager.PlayHaptic()`은 쿨타임 가드만 든 스텁이었고 **호출처가 0건**이었다(2026-08-07 개편 배경 ③). 두 갈래로 나눠 처리했다.

* **지금 처리** — `PlayerBall.OnCollisionEnter2D`에서 `FeedbackManager.Instance.PlayHaptic(collision.relativeVelocity.magnitude)` 호출. 세기 인자로 충돌 상대속도를 넘긴다. 연타 방지는 `FeedbackManager` 내부의 0.1초 쿨타임이 그대로 담당(TDD §8).
* **지금 처리** — `FeedbackManager.cs:30`의 상시 `Debug.LogWarning` 제거. 호출처가 생긴 이상 충돌마다(쿨타임 탓에 최대 초당 10회) 경고가 찍힐 자리였다.
* **Phase 13 이월** — 세기 **스케일링**. `Handheld.Vibrate()`엔 세기 파라미터가 아예 없어 Android `VibrationEffect` / iOS Core Haptics 연동이 선행돼야 한다. Phase 13에 이미 동일 항목(`충돌 상대속도에 비례한 SFX 볼륨 · 햅틱 세기 스케일`)이 있어 중복 정리를 겸했다.

#### 3. 검증 (플레이 모드, MCP)

에디터에서 `Handheld.Vibrate()`는 no-op이고 프레임도 흐르지 않아, **`Physics2D.simulationMode`를 `Script`로 바꿔 `Physics2D.Simulate()`로 물리를 직접 스텝**하고 `_lastFeedbackTime`(private)을 리플렉션으로 관측했다.

| # | 시나리오 | 결과 |
|---|---|---|
| 1 | 초기 → 60스텝(공 낙하·벽 충돌) | `-1` → `0` — **호출됨** |
| 2 | 센티넬 `-999` + 공에 속도 `(6, 4)` 부여 | `-999` → `0.02` — **호출됨** |
| 3 | 쿨타임 활성(`9999`) + 공에 속도 `(-6, 4)` 부여 | `9999` 유지 — **쿨타임이 실제로 차단** |

정지 상태에서는 `OnCollisionEnter2D`가 더 이상 발생하지 않아 갱신이 없다는 것도 함께 확인했다(Enter는 새 접촉에만 발생). 충돌 다발 플레이 후 콘솔 **에러/경고 0건** — 상시 `LogWarning` 제거도 이것으로 확인된다.

#### 4. 해결된 이슈 — 승/패 동시 발생 우선순위 (`[Doc]`)

* **결정: 현행 유지(first-event-wins), 엄격 적용하지 않는다.** GDD §3.3의 규칙은 "패배 우선"이지만, 현 구현은 `if (State != Play) return` 가드라 먼저 호출된 콜백이 이긴다(콜백 순서는 비결정적).
* **근거**: 동일 물리 스텝에 "마지막 공 골인"과 "다른 공의 이탈/피격"이 겹쳐야 성립하는 극단적 엣지다. 엄격 적용(`WaitForFixedUpdate` 보류 후 확정)은 골인 확정을 한 스텝 지연시켜 코어 루프의 즉시성을 해치고 Fast Retry 상태 복원과도 얽힌다.
* **기록**: `TDD.md §7.6`에 "규칙은 패배 우선이나 **현 구현은 이를 보장하지 않는다**"로 명시하고 재도입 절차를 남겼다(TDD v1.5). 결정을 숨기지 않고 한계로 드러내는 쪽을 택한 것이다.

#### 5. Phase 3 종료 — DoD 게이트

미완 0건 / `[QA]` 2건 존재 / `[x]` 전 항목 근거 보유 / 완료 조건 관측됨(2026-08-08 실플레이) / 태그 위반 없음 → **통과**.

#### 6. Phase 4 종료 — 좁은 통로 교착 검증

Phase 4에 유일하게 남아 있던 `[QA]` 좁은 통로 교착(끼임) 플레이테스트를 **사용자가 직접 플레이테스트해 통과**시켰다(2026-08-15). 2026-08-07 개편 때 "인터랙티브 플레이테스트 필요"라는 주석을 근거로 미완 처리했던 항목이라, 인터랙티브 검증으로만 닫을 수 있었다.

이로써 Phase 4의 완료 조건("색이 맞는 Goal에만 골인하고, 모든 공이 서로 충돌한다")이 전부 관측된 상태가 됐고, **Phase 8(색상 게이트)의 선행이 열렸다.** 향후 병목을 좁힐 때의 설계 기준은 `TDD §5.3.1`(위험 구간 `W = 1.05~1.25`, 안전은 `≥1.3` 또는 `≤0.9`)이며, 현재 마찰 값(공 `0.1` / 벽 `0.2`)이 낮아 아치가 잘 유지되지 않는다는 점도 함께 기록돼 있다 — 병목을 좁히면서 마찰을 같이 올리지 말 것.

**결과: Phase 1~7이 전부 닫혔다. 다음은 Phase 8(색상 게이트 · USP).**

---

### 📅 [2026-08-15] 씬 셋업 전수 실측(`/qa-scene`) — 카메라 `orthographicSize` 정정 · Phase 5 종료
...
#### 5. Phase 5(레벨 오소링 파이프라인) 종료 — `/phase-close`

카메라 항목이 Phase 5의 **마지막 미완 항목**이었다. DoD 게이트 5항 전부 통과:

| 게이트 | 결과 |
|---|---|
| 미완 항목 | 0건 (7항 전부 `[x]`) |
| `[QA]` 존재 | 2건 (동적 기믹 독립 콜라이더 워크플로우 / Composite 병합 실측) |
| 실측 근거 | 전 항목 근거 보유 — `StageData` 배선·SpriteShape 도입·곡선 벽 저작은 당일 `/qa-scene` 실측으로 재확인 |
| 완료 조건 | "곡선·비정형 지형을 표준 절차로 만들어 Composite에 병합할 수 있다" — TDD §6.1 절차 확립 + `RoundWall_0` 실제 저작·병합 유지 확인으로 관측됨 |
| 태그 규칙 | 위반 없음 |

* **스테일 표기 1건 정정**: Composite 병합 실측 항목이 가리키던 `Wall_SpriteShape_01`은 현 씬에 없다(2026-08-06 저작을 거쳐 `RoundWall_0`으로 대체). 과거 실측 기록 자체는 보존하고 현행 이름·현재 수치를 병기했다.
* **⚠️ Phase 8 선행 미충족**: Phase 8의 선행은 **Phase 4**인데, Phase 4에 `[QA]` 좁은 통로 교착 플레이테스트 1건이 미완으로 남아 있다. Phase 3(피드백 배선 · 승패 동시 발생 `[Doc]`)도 미완이나 Phase 8의 선행은 아니다.
...
`Assets/Scripts/UI/SafeAreaFitter.cs` 신설: `Screen.safeArea`(픽셀)를 부모 기준 정규화 anchor로 변환해 부착된 RectTransform에 적용. 앵커만 옮기므로 자식들(타이머·일시정지 버튼)의 상대 배치가 그대로 따라온다. `OnEnable`에서 적용하고, `Update`에서 마지막 적용 Rect와 달라졌을 때만 재적용(기기 회전·폴더블 대응 — struct 비교라 GC 없음, CLAUDE.md §4 준수).

**부착은 `HUD` 루트에만.** 노치에 걸릴 위험이 있는 건 상단의 타이머·일시정지 버튼뿐이고, `PausePanel`/`ResultPanel`의 dim 배경은 오히려 노치 영역까지 전체를 덮어야 자연스럽다(내용물은 중앙 배치라 노치와 무관). CLAUDE.md §6의 "최상단 UI 패널" = HUD.

설계 메모: `Apply(Rect)`가 인자를 받는 구조로 두어, 에디터(노치 없음)에서도 가짜 inset을 주입해 수식을 검증할 수 있게 했다.

#### 2. 검증 (MCP, 플레이 모드 1080×1920)

| 시나리오 | 결과 |
|---|---|
| 에디터 노옵 | `safeArea` = 전체 화면 → 앵커 `(0,0)-(1,1)` 불변 |
| 가짜 inset (상단 126px·하단 63px) | `anchorMin=(0, 0.033)` `anchorMax=(1, 0.934)` — 기대값과 일치 |
| 실제 `safeArea`로 복원 | `(0,0)-(1,1)` 복귀 |

부착 전 HUD 루트가 풀스트레치 앵커(오프셋 0)임을 확인 후 부착, 씬 저장. 콘솔 에러/경고 0건. **실물 노치 확인은 Phase 14 실기기 QA.**

#### 3. Phase 7 종료

이로써 Phase 7의 완료 조건("클리어/실패가 UI로 표시되고, 유저가 버튼으로 재시작·일시정지할 수 있다")이 사용자 실플레이로 확인된 상태에서 전 항목이 닫혔다. 프리팹화 항목만 실제 필요 시점인 **Phase 11로 이월**(로비 씬 신설 시 재사용). 다음은 **Phase 8 색상 게이트(USP)**.

* **해결된 이슈**:
  * Safe Area 대응 구현·부착·수식 검증 — Phase 7 마지막 빌드 항목
  * Phase 7 ✅ 종료, 현재 위치를 Phase 8로 갱신

---

### 📅 [2026-08-08] Phase 7 — 사용자 플레이테스트 통과 · **클리어 타임 0.1초 표시 불일치 수정**

#### 1. 플레이테스트 결과 (사용자 실플레이)

Phase 7에서 MCP로 검증 불가였던 항목들을 사용자가 에디터 실플레이로 확인했다. **결함 1건 외 전부 통과**:

* 일시정지: 타이머·물리 완전 정지, 일시정지 중 드래그 무시, **재개 시 미로 튐 없음**(입력 잠금 실작동), 일시정지 중 재시작 정상
* 클리어 → 1.5초 후 결과 팝업 → `[다시하기]` 재시작 정상, 별 개수 기준 일치
* 실패(장애물/장외) → **팝업 없이 즉시** 복원(GDD §2), 잔여 속도 없음 — Phase 3의 "실제 실패 경로" QA도 이것으로 통과
* HUD(타이머) 위 드래그는 회전 통과, 일시정지 버튼 위 드래그는 차단

#### 2. 🐞 발견 결함 — 결과 팝업의 클리어 타임이 HUD보다 0.1초 크게 표시

**증상**: HUD 타이머 최종 표시가 `8.8`인데 팝업은 `8.9s`.

**원인**: 같은 값을 두 곳이 다르게 깎는다.

* HUD(`HUDController`): `Mathf.FloorToInt(t * 10f)` — **버림** → 8.87 → `8.8`
* 팝업(`ResultPopupController`): 원시값을 `SetText("{0:1}s", t)`로 전달 — **TMP의 `{0:1}` 포맷은 반올림**이다. 패키지 소스(ugui 2.0.0 `TMP_Text.cs` `AddFloatToInternalTextBackingArray`)에서 포맷 직전 `valueD += k_Power[precision]`(소수 1자리면 +0.05) 후 자름 → 8.87 → `8.9`

클리어 순간의 백분위 자리가 5~9면(약 절반 확률) 정확히 0.1 어긋난다. 8-07의 R3 검증(12.34 → `12.3s`)은 우연히 내림되는 값이라 결함을 통과시켰던 것.

**수정** (1줄): 팝업을 HUD와 동일한 버림으로 통일 — 경과 타이머는 "실제보다 크게 표시하지 않는다"(버림)가 관례고 HUD가 이미 그렇게 돼 있으므로 팝업을 HUD에 맞추는 것이 최소 변경.

```csharp
_timeText.SetText("{0:1}s", Mathf.FloorToInt(clearTime * 10f) * 0.1f);
```

**검증** (MCP 플레이 모드, `HandleStageCleared` 직접 호출 5케이스): 8.87→`8.8s` · 8.8→`8.8s` · 8.94→`8.9s` · 12.34→`12.3s` · 14.97→`14.9s` — 전부 HUD 버림 값과 일치. 콘솔 에러/경고 0건.

#### 3. 관찰 기록 (반영하지 않음)

별 계산(`CalculateStars`)은 원시값을 쓰므로 경계에서 표시와 어긋날 수 있다 — 예: 15.04초 클리어 시 `15.0s`로 표시되는데 별은 2개(3별 기준 ≤15.0). 버림/반올림 어느 쪽이든 존재하는 경계 현상이고, 없애려면 HUD까지 올림(ceil)으로 바꿔야 한다. 발생 확률이 낮아 관찰만 남긴다.

* **해결된 이슈**:
  * Phase 7 잔여 QA 2건 + Phase 3 실제 실패 경로 QA — 사용자 실플레이로 통과, `[x]` 처리
  * 클리어 타임 표시 불일치(버림 vs 반올림) 수정 및 5케이스 회귀 검증
  * Phase 7 잔여 항목: Safe Area(`[Code]`+`[Editor]`) · HUD/팝업 프리팹화(`[Editor]`, Phase 11 연계)

---

### 📅 [2026-08-08] Task.md 종합 검토 — 완료 표기 전수 실측 통과 · **프로젝트 목표 우선순위 확정(포트폴리오 1차)** 및 계획 교정

#### 1. 검토 방법과 결과 — 이번엔 문서가 옳았다

2026-08-07에 전면 개편한 `Task.md`가 신뢰 가능한지, 완료 표기를 에디터 실행 없이 **파일 레벨 실측**(YAML/설정 파일 직접 판독 + 전체 grep)으로 재검증했다. **확인한 전 항목에서 표기와 실제가 일치** — 개편 때 도입한 DoD가 실제로 작동하고 있다.

| 검증 대상 | 방법 | 결과 |
|---|---|---|
| Layer Collision Matrix | `Physics2DSettings.asset`의 `m_LayerCollisionMatrix` 비트 직접 디코딩 | 레이어6 마스크 `0xFFFFFEFF`(bit 8만 OFF = Ball_Yellow×Gate_Yellow 통과), 레이어7 `0xFFFFFDFF`(bit 9 OFF), 레이어 8·9 대칭 일치, 공-공 전 교차 ON — 스펙(커밋 `d23ec99`)과 일치 |
| FeedbackManager 데드코드 | 전체 grep | `PlayHaptic` 호출처 0건 — Phase 3 재개방 표기가 옳음 |
| 씬 카메라 오설정 | `SampleScene.unity` 직접 판독 | 컴포넌트 저장값 `6` vs 런타임 `defaultOrthoSize: 10.5` 불일치 실재 — Phase 5 미결 표기가 옳음 |
| 색 레이어 / 게이트 | TagManager + 씬·프리팹 | 레이어 6~9 정의만 존재, `PlayerBall.prefab`의 `m_Layer: 0`(Default), 씬에 Gate 오브젝트 0개 |
| 공·골 색 매칭 | 씬 프리팹 오버라이드 판독 | 공 `_colorType` 3(Green)·4(Yellow) + Goal 3·4 쌍 일치 |
| 기타 | — | `PlayerBall.prefab`에 `Player` 태그 확인(DeadZone/Obstacle 판정 성립), `Assets/Data/Stage 1.asset` 존재, Build Settings 등록 씬 1개 |

Phase 7 코드(`GameManager`·`HUDController`·`ResultPopupController`·`InputController`)도 문서 서술과 일치하며, CLAUDE.md §4 기준 위반 없음(Update 무할당 · 이벤트 해제 · 캐싱 · `TryGetComponent` 등).

#### 2. 프로젝트 목표 우선순위 확정 — 이후 모든 스코프 판단의 기준

**1차 = 개발 경험·포트폴리오, 2차 = 실제 출시 시도** (대학생 개발자, 2026-08-08 사용자 확정). 이 결정이 아래 계획 교정의 근거다.

* 포트폴리오 핵심 증거물: **실기에서 도는 시연 영상 + USP 기믹 + 개발 과정 기록**(이 문서 체계 자체가 자료다)
* 광고 SDK·IAP·현지화 등 출시 전용 작업은 역량 신호가 약함 → 연기/생략 가능해야 함 → `Task.md`에 `🚀[출시]` 트랙 도입

#### 3. 반영한 변경

**`Task.md`**
1. **DoD 규칙 6 정정**: "TDD가 우선" → **기획 충돌은 GDD, 구현 스펙 충돌은 TDD 우선**. 구 문구는 이 문서 자신의 선례(8-07 GameOver 즉시 재시작 건 — GDD §2 근거로 정정)와 어긋났다.
2. **§트랙 신설**: `🚀[출시]` = 출시 전용, 연기해도 포트폴리오 무손실. **Phase 12 전체** + Phase 14의 분석 본 연동·현지화에 적용.
3. **Phase 3**: 실제 실패 경로(DeadZone/Obstacle 충돌→복원) 플레이 QA 신설 — 기존 T8은 `GameOver()`를 스크립트로 직접 호출한 것이라 "충돌→판정" 경로는 미검증. 햅틱 세기 항목에 `Handheld.Vibrate()` 세기 파라미터 부재 제약 메모.
4. **Phase 8 끝에 실기 스모크 테스트 신설**: GDD §11.2가 회전 멀미(최상위 리스크)에 "조기 실기 테스트"를 권고하는데 구 계획은 실기 검증이 전부 Phase 14에 몰려 있었다 — 계획이 GDD와 어긋난 지점 ①.
5. **Phase 10 선행 완화 + 로컬 실패 로깅 앞당김**: 초반(단일 공) 레벨은 Phase 8 이후 착수 가능(GDD §6.3 — 초반 레벨은 동적 기믹 불요, Phase 9와 병렬 허용). 실패율 데이터의 용처(GDD §10 리밸런싱)가 Phase 10 튜닝인데 분석이 Phase 14면 데이터 없이 튜닝하게 됨 — 어긋난 지점 ②.
6. **Phase 14에 🎓 포트폴리오 최종 산출물 항목**(실기 빌드 + 시연 영상/GIF + README) — 출시가 무산돼도 1차 목표는 확보되는 안전망.
7. TDD **라인 번호** 참조 4건을 섹션 참조로 교체 — 아래 TDD 수정으로 라인이 밀리는 데다 원래 깨지기 쉬운 참조였다.

**`TDD.md` (v1.4)** — 8-07 Task 재번호를 반영 못 한 스테일 참조 정정: §8 풀링 "Task Phase 7"→**14**, §10 StageLoader "Phase 5"→**11**, §5.2 "Phase 4 게이트 구현 시"→**8** + Matrix 배선 완료(8-07) 반영, §11 `FeedbackManager` `✅`→`⚠️ 스텁(호출처 0건)`.

**`GDD.md` (v2.0.1)**: 헤더 진척("Phase 1~3 완료"→신 번호 기준 1~6 완료·7 진행), §9 BGM "(Task Phase 6)"→**13**, §11.3 "Phase 1~7"→**1~14**.

**`CLAUDE.md`**: §6 풀링 "(Phase 7…)"→"(Task Phase 14…)" — 검토 중 추가 발견분.

#### 4. 발견했으나 반영하지 않은 것 (관찰 기록)

* **Clear 연출 1.5초 동안 회전 입력이 잠기지 않는다**: `OnBallReachedGoal()`이 `ResetInput()`만 호출하고 `SetInputEnabled(false)`는 안 한다. 공이 전부 비활성이라 게임플레이 영향은 없지만, 연출 중 드래그하면 화면이 돈다. 기획 요구가 아니라 미처리 — 거슬리면 Phase 13 폴리싱에서.
* Task.md 정정 목록의 "TDD 스테일 3건" 중 BGM 참조는 실제로는 **GDD §9**에 있었고, 목록에 없던 스테일 참조가 TDD §10·§5.2·CLAUDE.md §6에서 추가로 나왔다(전부 정정함). Safe Area "(Task Phase 7)"은 신 번호로도 7이라 변경 불요.

* **해결된 이슈**:
  * Task.md 완료 표기의 신뢰성을 실측으로 재확인 — "완료 표기 ≠ 실제" 사고의 재발 없음
  * 계획이 GDD와 어긋난 2곳(조기 실기 테스트 부재 · KPI 데이터가 용처보다 늦음) 교정
  * 문서 정합성 미결 3건 + 추가 발견 스테일 참조 일괄 정정 — 4개 문서의 Phase 번호가 전부 신 번호로 통일됨
  * 프로젝트 목표 우선순위(포트폴리오 1차)를 문서에 명시 — 이후 스코프 판단 기준 확립
